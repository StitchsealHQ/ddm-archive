...
REL_PROMPT = """다음은 '동대문 아카이브'(서울 동대문 패션타운·도매시장·DDP·동대문구 상권·관광 정보 수집)에
수집된 자료 목록이다. 각 항목은 "id: [카테고리] 제목 | 코멘트" 형식이다.

동대문과 명백히 무관한 항목의 id만 골라라. 예: 연예인 근황, 동대문 언급 없는 일반 경제·유통 기사,
타지역 행사·맛집(남대문·종로·강남 등 다른 상권 단독), 인사·부고, 무관 블로그 일상글.

주의 — 다음은 동대문 '관련 있음'이므로 제외하면 안 된다:
...
- 흥인지문·창신동·평화시장 등 동대문 일대 지역·관광 콘텐츠
제목만으로 판단이 불확실하면 관련 있는 것으로 간주하고 제외하지 마라
(동대문 관련 검색으로 수집된 자료라 본문에 동대문이 언급될 수 있다).
